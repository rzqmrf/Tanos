...
<div class="space-y-6 w-full" x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button @click="showCreateModal = true"
                ...
                class="px-4 py-2 hover:opacity-90 active:scale-95 text-white text-xs font-bold rounded-xl shadow-sm transition flex items-center space-x-1.5 self-start sm:self-auto cursor-pointer border-0">
                                ...
                                <div class="flex items-center justify-center space-x-1.5">
                                    @if($category === 'wbs_payroll_category')
                                        <a href="{{ route('project.master.wbs-payroll-categories.show', $item->id) }}" 
                                           ...
                                           class="p-1.5 text-white rounded-lg hover:opacity-90 transition shadow-xs cursor-pointer" title="Lihat Detail">
                                    ...
                                    <button @click="openEdit({{ $item->toJson() }})" 
                                            ...
                                            class="p-1.5 text-white rounded-lg hover:opacity-90 transition shadow-xs cursor-pointer" title="Edit Data">
                                    ...
                                    <form action="{{ route('project.master.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')" class="inline">
                                        ...
                                        <button type="submit" 
                                                ...
                                                class="p-1.5 text-white rounded-lg hover:opacity-90 transition shadow-xs cursor-pointer" title="Hapus Data">
    ...
    <div x-show="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/65 backdrop-blur-xs" style="display: none;">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative" @click.away="showCreateModal = false">
            ...
            <form action="{{ route('project.master.store') }}" method="POST" class="mt-4 space-y-4">
                ...
                <div class="flex items-center justify-end space-x-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button type="submit" style="background-color: #00c853; color: #ffffff;" class="px-4 py-2 hover:opacity-90 text-white rounded-xl text-xs font-bold shadow-sm transition cursor-pointer border-0">Simpan Data</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative" @click.away="showEditModal = false">
            ...
            <form :action="'/projects/master/items/' + editData.id" method="POST" class="mt-4 space-y-4">
                ...
                <div class="flex items-center justify-end space-x-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button type="submit" style="background-color: #7c3aed; color: #ffffff;" class="px-4 py-2 hover:opacity-90 text-white rounded-xl text-xs font-bold shadow-sm transition cursor-pointer border-0">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
