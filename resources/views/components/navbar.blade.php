    ...
    <div class="flex items-center space-x-3 lg:space-x-4">
        ...
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            <button type="button" @click="open = !open"
                    ...
                    class="flex items-center space-x-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 pl-1.5 pr-3 py-1 rounded-full hover:border-slate-300 dark:hover:border-slate-700 shadow-sm transition-all duration-200 group">
                <!-- Avatar -->
                @if($dbUser && $dbUser->photo)
                    <img src="{{ asset('storage/' . $dbUser->photo) }}" alt="Avatar" class="w-7 h-7 rounded-full object-cover nav-avatar-img ring-2 ring-transparent group-hover:ring-blue-500 transition">
                @else
                    <div class="w-7 h-7 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center font-bold text-[11px] select-none nav-avatar-placeholder shadow-inner">
                        {{ $navInitials }}
                    </div>
                @endif
                <span class="text-xs font-bold text-slate-700 dark:text-slate-200 hidden md:block max-w-[90px] truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition">{{ $navName }}</span>
            ...
            <div x-show="open"
                ...
                <div class="p-2">
                    ...
                    <a href="#" @click.prevent="open = false; $dispatch('open-settings-modal')" class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/60 hover:text-slate-800 dark:hover:text-slate-100 transition-colors group">
                        <div class="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center group-hover:bg-blue-50 dark:group-hover:bg-blue-950/40 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="w-full">
                        @csrf
                        <button type="submit" class="w-full text-left flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-semibold text-rose-500 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-colors group">
