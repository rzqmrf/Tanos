...
<header class="sticky top-0 bg-[#f8fafc]/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-900 z-30 px-6 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center space-x-3 lg:space-x-4">
        ...
        <!-- User Profile Button (Directly Opens Profile Modal) -->
        @php
            $dbUser = null;
            if (auth()->check()) {
                $dbUser = auth()->user();
        ...
        <div class="relative">
            <button type="button" @click="$dispatch('open-profile-modal')"
                    ...
                    class="flex items-center space-x-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 pl-1.5 pr-3 py-1 rounded-full hover:border-blue-500/50 dark:hover:border-blue-500/50 shadow-sm hover:shadow-md transition-all duration-200 group">
                <!-- Avatar -->
                ...
                <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse hidden md:block"></div>
            </button>
        </div>
    </div>
</header>
