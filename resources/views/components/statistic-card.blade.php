...
<div class="bg-white rounded-3xl p-5 border border-slate-100 shadow-[0_4px_15px_-3px_rgba(0,0,0,0.02)] hover:-translate-y-1 hover:shadow-[0_12px_30px_-5px_rgba(0,0,0,0.06)] hover:border-slate-200/60 transition-all duration-300 flex items-center group">
    ...
    <div class="min-w-0">
        ...
        <!-- Alpine reactive value binding -->
        <span x-text="stats.{{ $field }}" class="text-2xl font-extrabold text-slate-800 tracking-tight block leading-tight">
        ...
        <div class="flex items-center mt-1 text-[11px] font-bold"
                :class="stats.{{ $field }}Growth >= 0 ? 'text-emerald-600' : 'text-rose-500'">

            <!-- Up arrow SVG if positive -->
            <template x-if="stats.{{ $field }}Growth >= 0">
            ...
            <!-- Percentage value -->
            <span x-text="Math.abs(stats.{{ $field }}Growth).toFixed(1) + '%'"></span>
