...
@section('title', 'Tambah Parameter Rules — Tanos ERP')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <!-- Page Header -->
    <x-page-header 
        title="Tambah Parameter Rules Jam Kerja" 
        subtitle="Atur batas toleransi terlambat masuk, pulang cepat, dan tanggal keberlakuan aturan."
        :breadcrumbs="[
            'General' => '#',
            'Human Resource' => '#',
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden p-6 sm:p-8">
        <form action="{{ route('org.evaluations.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Aturan Standar Toleransi Absensi 2026" 
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="late_tolerance_minutes" value="{{ old('late_tolerance_minutes', 15) }}" required min="0" 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="early_departure_minutes" value="{{ old('early_departure_minutes', 15) }}" required min="0" 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="valid_from" value="{{ old('valid_from', date('Y-01-01')) }}" required 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="valid_to" value="{{ old('valid_to', date('Y-12-31')) }}" required 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <select name="is_active" required 
                        ...
                        <option value="1" {{ old('is_active', '1') == '1' ? 'selected' : '' }}>Ya (Aktifkan sebagai Acuan Utama Engine Rekap Absensi)</option>
                        <option value="0" {{ old('is_active') == '0' ? 'selected' : '' }}>Tidak (Draft / Cadangan)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" 
                    ...
                    <span>Simpan Parameter Rules</span>
                </button>
            </div>
        </form>
    </div>
