...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-2.5 font-medium">
                Katalog data induk seluruh pegawai, struktur organisasi, dan penempatan regional.
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden p-6 space-y-5">
        ...
        <!-- Filter Controls -->
        <form method="GET" action="{{ route('employees.index') }}" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center space-x-2">
                <div class="relative min-w-[240px]">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search nama, NIPP, NIK, jabatan..."
                ...
                <a href="{{ route('employees.index') }}" class="p-2 text-xs font-bold text-slate-400 hover:text-slate-600">Reset</a>
                @endif
            </div>
        </form>
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-50/80 dark:bg-slate-800/60 border-y border-slate-200 dark:border-slate-800 text-[11px] font-extrabold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                        <th class="py-3.5 px-3">NIPP</th>
                        <th class="py-3.5 px-3">Nama Pegawai</th>
                        <th class="py-3.5 px-3">Jabatan / Role</th>
                        <th class="py-3.5 px-3">Regional & Sub</th>
                        <th class="py-3.5 px-3">Segment</th>
                        <th class="py-3.5 px-3">Bank Account</th>
                        <th class="py-3.5 px-3 text-center">PTKP</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition">
                        ...
                        <td class="py-3.5 px-3 font-bold text-slate-900 dark:text-slate-100">
                            <a href="{{ route('employees.show', $item->id) }}" class="hover:text-primary dark:hover:text-sky-400 transition">
                                {{ $item->name }}
                            </a>
                        </td>
                        ...
                        <td class="py-3.5 px-3 text-slate-600 dark:text-slate-400">
                            {{ $item->regional }}
                            @if($item->sub_regional)
                                <span class="block text-[10px] text-slate-400 font-medium">({{ $item->sub_regional }})</span>
                            @endif
                        </td>
                        ...
                        <td class="py-3.5 px-3 font-mono text-slate-600 dark:text-slate-400">
                            @if($item->bank_account_number)
                                <span>{{ $item->bank_name ?? 'Bank' }}: {{ $item->bank_account_number }}</span>
                            @else
                        ...
                        <td class="py-3.5 px-3 text-center whitespace-nowrap">
                            <div class="inline-flex items-center space-x-1.5">
                                ...
                                <a href="{{ route('employees.show', $item->id) }}"
                                   class="p-2 rounded-lg bg-sky-500 hover:bg-sky-600 text-white transition shadow-2xs flex items-center justify-center" title="View Detail">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                </a>

                                <!-- Edit (Purple) -->
                                <a href="{{ route('employees.edit', $item->id) }}"
                                   class="p-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-white transition shadow-2xs flex items-center justify-center" title="Edit Employee">
                                ...
                                <form action="{{ route('employees.destroy', $item->id) }}" method="POST" class="inline"
                                    ...
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="p-2 rounded-lg bg-rose-600 hover:bg-rose-700 text-white transition shadow-2xs flex items-center justify-center cursor-pointer" title="Delete Employee">
