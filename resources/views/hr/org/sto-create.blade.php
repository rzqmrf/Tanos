...
@section('title', 'Tambah Unit Kerja Baru — Tanos ERP')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <!-- Page Header -->
    <x-page-header 
        title="Tambah Unit / Departemen Baru" 
        subtitle="Definisikan unit organisasi baru, tentukan induk hierarki, regional, serta unit type."
        :breadcrumbs="[
            'General' => '#',
            'Human Resource' => '#',
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden p-6 sm:p-8">
        <form action="{{ route('org.sto.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" required placeholder="Contoh: ORG-5001" 
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Divisi Operasi Pelabuhan" 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <select name="parent_id" 
                        ...
                        @foreach($rootDivs as $div)
                            <option value="{{ $div->id }}" {{ old('parent_id') == $div->id ? 'selected' : '' }}>
                                {{ $div->name }} ({{ $div->code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="unit_type" required 
                            class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-800 dark:text-slate-100 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition">
                        <option value="Division">Division / Direktorat</option>
                        <option value="Department">Department / Bagian</option>
                        <option value="Section">Section / Seksi</option>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <input type="text" name="cost_center" value="{{ old('cost_center') }}" 
                           placeholder="Contoh: CC-100201 (Divisi Operasi)" 
                           class="w-full text-xs px-3.5 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">
                </div>
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <textarea name="description" rows="3" 
                              placeholder="Tuliskan wewenang dan cakupan kerja unit organisasi ini..."
                              class="w-full text-xs p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-slate-800 dark:text-slate-100 transition">{{ old('description') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" 
                    ...
                    <span>Simpan Unit Organisasi</span>
                </button>
            </div>
        </form>
    </div>
