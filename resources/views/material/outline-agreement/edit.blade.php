...
@section('title', 'Outline Agreement - Edit — Tanos ERP')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <div class="flex items-center space-x-2 text-xs font-bold text-slate-400 dark:text-slate-400 mb-4.5">
                ...
                <span>Material</span>
                <span class="text-slate-300 dark:text-slate-600">/</span>
                <a href="{{ route('material.outline-agreement') }}" class="hover:text-primary dark:hover:text-sky-400 transition">Outline Agreement</a>
                <span class="text-slate-300 dark:text-slate-600">/</span>
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center space-x-3">
                <span>Outline Agreement - Edit</span>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-2.5 font-medium">
                Perbarui data klausul, masa berlaku, dan target nilai kontrak payung.
            </p>
        </div>

        <div class="flex items-center space-x-2 self-start sm:self-auto">
            <a href="{{ route('material.outline-agreement.show', $agreement->id) }}"
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8">
        <form action="{{ route('material.outline-agreement.update', $agreement->id) }}" method="POST" class="space-y-6 text-xs">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="partner_id" required
                            ...
                            @foreach($partners as $p)
                                <option value="{{ $p->id }}" {{ $agreement->partner_id == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->code }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="agreement_type" required
                                class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-slate-800 dark:text-slate-100 focus:outline-none">
                            <option value="Value Contract" {{ $agreement->agreement_type === 'Value Contract' ? 'selected' : '' }}>Value Contract (Berdasarkan Plafon Anggaran)</option>
                            <option value="Quantity Contract" {{ $agreement->agreement_type === 'Quantity Contract' ? 'selected' : '' }}>Quantity Contract (Berdasarkan Target Volume Unit)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-3 gap-3">
                        <div class="col-span-1">
                            ...
                            <input type="text" name="currency" value="{{ old('currency', $agreement->currency) }}" required
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="number" name="target_value" value="{{ old('target_value', $agreement->target_value) }}" min="0" required
                                   class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono font-bold text-slate-800 dark:text-slate-100 focus:outline-none">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', $agreement->start_date) }}" required
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" value="{{ old('end_date', $agreement->end_date) }}" required
                                   class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono text-slate-800 dark:text-slate-100 focus:outline-none">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="Active" {{ $agreement->status === 'Active' ? 'selected' : '' }}>Active (Berlaku)</option>
                            <option value="Draft" {{ $agreement->status === 'Draft' ? 'selected' : '' }}>Draft (Dalam Penyusunan)</option>
                            <option value="Expired" {{ $agreement->status === 'Expired' ? 'selected' : '' }}>Expired (Habis Masa Berlaku)</option>
                            <option value="Terminated" {{ $agreement->status === 'Terminated' ? 'selected' : '' }}>Terminated (Dihentikan)</option>
                        </select>
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">Syarat & Ketentuan Pembayaran</label>
                        <input type="text" name="terms" value="{{ old('terms', $agreement->terms) }}"
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full p-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-slate-100 focus:outline-none leading-relaxed">{{ old('notes', $agreement->notes) }}</textarea>
            </div>

            <!-- Submit -->
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('material.outline-agreement.show', $agreement->id) }}" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 font-bold rounded-xl transition">Batal</a>
