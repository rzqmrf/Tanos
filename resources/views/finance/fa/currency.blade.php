...
<div class="space-y-6" x-data="{
    ...
    <x-page-header 
        title="Currency Master (Mata Uang)" 
        subtitle="Master mata uang transaksi, simbol valas, dan penentuan mata uang dasar (Base Currency)."
        :breadcrumbs="[
            'General' => '#',
            'Master FA' => '#',
            'Currency' => ''
    ...
    <x-data-card 
        ...
        :search-route="route('fa.currency')"
    >
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        ...
                        <th class="py-3.5 px-5">Nama Mata Uang</th>
                        <th class="py-3.5 px-5 text-center">Simbol</th>
                        <th class="py-3.5 px-5 text-center">Mata Uang Utama</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs font-medium text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition">
                        ...
                        <td class="py-3.5 px-5 text-center font-bold text-slate-800 dark:text-slate-200">{{ $item->symbol }}</td>
                        ...
                        <td class="py-3.5 px-5 text-center whitespace-nowrap">
                            <div class="inline-flex items-center space-x-1.5">
                                ...
                                <x-action-button type="edit" :click="'openEdit(' . json_encode($item) . ')'" title="Edit Mata Uang" />
                                @if(!$item->is_default)
                                <form action="{{ route('fa.currency.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus Mata Uang {{ $item->code }}?')">
                                    @csrf
                                    @method('DELETE')
                                    <x-action-button type="delete" title="Hapus Mata Uang" />
                                </form>
                                @endif
                            </div>
                        </td>
