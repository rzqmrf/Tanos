    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm">
        <div>
            ...
            <h1 class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center">
                <span>Fiscal Period Management (Periode Pembukuan)</span>
            </h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Buka / Tutup buku periode akuntansi bulanan untuk memvalidasi transaksi jurnal & posting payroll.</p>
        </div>
        ...
        <div class="flex items-center space-x-2">
            ...
            <form method="GET" action="{{ route('fa.period') }}" class="flex items-center space-x-2">
                <select name="year" onchange="this.form.submit()"
                        class="px-3.5 py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 font-bold text-xs rounded-xl focus:outline-none cursor-pointer">
