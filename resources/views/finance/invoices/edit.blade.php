...
@section('title', 'Invoice - Edit — Tanos ERP')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1.5 font-medium">
                Perbarui data tipe tagihan, periode bulan, dan nominal faktur.
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8">
        <form action="{{ route('invoices.update', $invoice->id) }}" method="POST" class="space-y-6 text-xs max-w-3xl">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="P2P" {{ $invoice->type === 'P2P' ? 'selected' : '' }}>P2P (Procure-to-Pay / Vendor Tagihan)</option>
                        <option value="Non P2P" {{ $invoice->type === 'Non P2P' ? 'selected' : '' }}>Non P2P (Operasional / Direct Billing)</option>
                    </select>
                </div>
                    ...
                    <div>
                        ...
                        <select name="regional" required
                            ...
                            @foreach($regionals as $r)
                                <option value="{{ $r->name }}" {{ $invoice->regional === $r->name ? 'selected' : '' }}>{{ $r->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="segment" required
                            ...
                            @foreach($segments as $s)
                                <option value="{{ $s->name }}" {{ $invoice->segment === $s->name ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="month" required
                                class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-slate-800 dark:text-slate-100 focus:outline-none">
                            @foreach($months as $m)
                                <option value="{{ $m }}" {{ $invoice->month === $m ? 'selected' : '' }}>{{ $m }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" value="{{ old('amount', $invoice->amount) }}" min="0" required
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('invoices.show', $invoice->id) }}" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 font-bold rounded-xl transition">Batal</a>
