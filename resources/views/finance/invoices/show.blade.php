...
@section('title', 'Invoice - View — Tanos ERP')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <div class="flex items-center space-x-2 text-xs font-bold text-slate-400 dark:text-slate-400 mb-4.5">
                ...
                <span class="text-slate-300 dark:text-slate-600">/</span>
                <a href="{{ route('invoices.index') }}" class="hover:text-primary dark:hover:text-sky-400 transition">Invoice</a>
                <span class="text-slate-300 dark:text-slate-600">/</span>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-2.5 font-medium">
                Detail data faktur tagihan dan rincian alokasi regional.
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 space-y-6 max-w-4xl">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
            <div class="flex items-center space-x-3">
                ...
                <span class="px-2.5 py-0.5 rounded-lg text-xs font-extrabold {{ $invoice->type === 'P2P' ? 'bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300' : 'bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300' }}">
                    {{ $invoice->type }}
                </span>
            </div>

            <div class="font-mono text-xs text-slate-400 font-medium">
                Created: {{ $invoice->created_at ? $invoice->created_at->format('d M Y H:i') : '-' }}
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div class="space-y-4">
                <div>
                    <label class="block font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Regional Penempatan</label>
                    <div class="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl font-bold text-slate-800 dark:text-slate-200">{{ $invoice->regional }}</div>
                </div>

                <div>
                    <label class="block font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Segment Bisnis</label>
                    <div class="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl font-semibold text-slate-800 dark:text-slate-200">{{ $invoice->segment }}</div>
                </div>
            </div>

            <div class="space-y-4">
                <div>
                    <label class="block font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Periode Bulan Tagihan</label>
                    <div class="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl font-semibold text-slate-800 dark:text-slate-200">{{ $invoice->month }}</div>
                </div>

                <div>
                    <label class="block font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Total Nominal Tagihan</label>
                    <div class="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl font-mono font-black text-base text-primary">
